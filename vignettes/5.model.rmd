...
### Model Selection
Several simple linear models were fit to determine which of the following quantititive variables in `r kfigr::figr(label = "slr_variables", prefix = TRUE, link = TRUE, type="Table")` was the best predictor of the log of daily box office revenue.
...
As suggested by the correlation analysis in `r kfigr::figr(label = "correlation1", prefix = TRUE, link = TRUE, type="Table")` and summarized in `r kfigr::figr(label = "modelOne_comparison", prefix = TRUE, link = TRUE, type="Table")` the log number of IMDB votes was the best predictor of the log of daily box office revenue (F(`r modelOne$best$anova$Df[1]`, `r modelOne$best$anova$Df[2]`) =  `r round(modelOne$best$anova[1,5], 3)`, p <  `r ifelse(modelOne$best$anova[1,6][1] < .001, ".001", ifelse(modelOne$best$anova[1,6][1] < .01, ".01", ifelse(modelOne$best$anova[1,6][1] < .05, ".05", round(modelOne$best$anova[1,6][1], 3))))`), with an adjusted R-Squared of `r round(modelOne$best$glance[1,9], 3)`. The model accounted for `r round(modelOne$best$glance[1,11], 0)`% of the variance in the response.
...
```{r modelOne_linearity, fig.height=20}
...
`r modelOne$best$comment$linearity`
...
`r modelOne$best$comments$homoscedasticity`
...
`r modelOne$best$comments$normality`
...
`r modelOne$best$comments$outliers` A case-wise review of the influential points did not reveal any data quality issues; therefore, the influential points were not removed from the model. 

### Model Interpretation
The final prediction equation was defined as follows:  
$y_i$ = `r round(modelOne$best$coefficients$estimate[1], 2)` 
+ `r round(modelOne$best$coefficients$estimate[2], 2)`$x_1$
+ $\epsilon$    

where:  
$x_1$ is `r modelOne$best$coefficients$term[2]`   
...
`r modelOne$best$comments$anova`

#### Interpretation of Coefficients
The intercept `r round(modelOne$best$coefficients$estimate[1], 2)` is the prediction of log daily box office revenue for a film where the log number of IMDB votes is zero. The prediction of the log daily box office (in log dollars) is therefore, `r round(modelOne$best$coefficients$estimate[1], 2)` plus `r round(modelOne$best$coefficients$estimate[2], 2)` log dollars of daily box office revenue for each log IMDB vote.
...
# Obtain Data
mData <- process(train, group = "c", y = "imdb_num_votes_log")  
...
gridExtra::grid.arrange(gridExtra::arrangeGrob(modelA$plots$linearity[[1]],
                        ...
                        gridExtra::arrangeGrob(modelA$plots$linearity[[9]],
                                               modelA$plots$linearity[[10]], ncol = 2),
                         nrow = 4)
...
# Obtain Data
mData <- process(train, group = "c", "imdb_num_votes_log", outliers = modelA$tests$influential)  
...
# Obtain Data
mData <- process(train, group = "c", "imdb_num_votes_log")  
...
# Obtain Data
mData <- process(train, group = "c", "imdb_num_votes_log", outliers = modelC$tests$influential)  
...
knitr::kable(accuracy, digits = 3) %>%
  kableExtra::kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = T, position = "center")
```
There were no significant differences in MAPE, MSE, and RMSE between the models.  The negative MPE indicated that all models were biased with over predictions. From a percent accuracy perspective, it is worth noting that the forward selection and backward selection models performed nearly identically with and without the influence points.  That said, the models *with* the influence points had `r round((sum(accuracy[1,11], accuracy[3,11]) / sum(accuracy[2,11], accuracy[4,11]) - 1), 2)` greater prediction accuracy. Having the highest percent accuracy, the Alpha model would advance to the prediction stage.
...
+ `r round(modelA$coefficients$estimate[29], 3)`$x_{28}$
+ `r round(modelA$coefficients$estimate[30], 3)`$x_{29}$
+ `r round(modelA$coefficients$estimate[31], 3)`$x_{30}$
+ `r round(modelA$coefficients$estimate[32], 3)`$x_{31}$
+ `r round(modelA$coefficients$estimate[33], 3)`$x_{32}$
+ $\epsilon$
...
$x_{17}$ is `r modelA$coefficients$term[18]`
$x_{18}$ is `r modelA$coefficients$term[19]`
$x_{19}$ is `r modelA$coefficients$term[20]`
$x_{20}$ is `r modelA$coefficients$term[21]`
$x_{21}$ is `r modelA$coefficients$term[22]`
$x_{22}$ is `r modelA$coefficients$term[23]`
$x_{23}$ is `r modelA$coefficients$term[24]`
$x_{24}$ is `r modelA$coefficients$term[25]`
$x_{25}$ is `r modelA$coefficients$term[26]`
$x_{26}$ is `r modelA$coefficients$term[27]`
$x_{27}$ is `r modelA$coefficients$term[28]`
$x_{28}$ is `r modelA$coefficients$term[29]`
$x_{29}$ is `r modelA$coefficients$term[30]`
$x_{30}$ is `r modelA$coefficients$term[31]`
$x_{31}$ is `r modelA$coefficients$term[32]`
$x_{32}$ is `r modelA$coefficients$term[33]`
...
* plus `r round(subset(modelA$coefficients, term == "cast_votes_log", select = estimate), 3)` log IMDB votes for each log IMDB vote previously earned by the cast members,
* plus `r round(subset(modelA$coefficients, term == "critics_score", select = estimate), 3)` log IMDB votes for each point of the Rottentomatoes critics score,
* plus `r round(subset(modelA$coefficients, term == "runtime_log", select = estimate), 3)` log IMDB votes for each minute of runtime,
* plus a number of log IMDB votes for the associated MPAA rating,
* plus `r round(subset(modelA$coefficients, term == "best_pic_nomyes", select = estimate), 3)` log IMDB votes if the film was nominated for an oscar for best film,
* plus `r round(subset(modelA$coefficients, term == "best_pic_winyes", select = estimate), 3)` log IMDB votes if the film won an oscar for best picture,
* plus `r round(subset(modelA$coefficients, term == "best_actor_winyes", select = estimate), 3)` log IMDB votes if the film won an oscar for best actor,
* plus a number of log IMDB votes associated with the month of theatrical release,
* plus `r round(subset(modelA$coefficients, term == "director_experience_log", select = estimate), 3)` log IMDB votes for each film directed by the film's director.

## Model Summary
The purpose of this section was to develop a model that would be able to predict "box office success". Given the signficant right skew in box office revenue, the log of box office revenue became the proxy for box office success. Therefore, two regression models were fit in this section.  Model One, the simple linear regression model (F(`r modelOne$best$glance[1,3]`, `r modelOne$best$glance[1,4]`) = `r round(modelOne$best$glance[1,5],2)`, p < .001) showed that the log number of IMDB votes was the best predictor of the log of box office revenue.  Designated log IMDB votes as the response variable, Model Two (F(`r modelA$glance[1,3]`, `r modelA$glance[1,4]`) = `r round(modelA$glance[1,5], 2)`, p < .001) was selected from among four multiregression linear models employing forward selection and backward elimination algorithms. Next, the models will be used to predict the number of log IMDB votes and the log box office for a randomly selected film.
