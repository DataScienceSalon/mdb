...
* plus `r round(subset(modelA$coefficients, term == "director_experience_log", select = estimate), 3)` log IMDB votes for the log unit of films directed by the film's director.

It should be noted that the influence of cast votes and the log of director experience was not significant.

## Model Summary
The purpose of this section was to develop a model that would be able to predict "box office success". Given the signficant right skew in box office revenue, the log of box office revenue became the proxy for box office success. Therefore, two regression models were fit in this section.  Model One, the simple linear regression model (F(`r modelOne$glance[1,3]`, `r modelOne$glance[1,4]`) = `r round(modelOne$glance[1,5],2)`, p < .001) showed that the log number of IMDB votes was the best predictor of the log of box office revenue.  Designated log IMDB votes as the response variable, Model Two (F(`r modelA$glance[1,3]`, `r modelA$glance[1,4]`) = `r round(modelA$glance[1,5], 2)`, p < .001) was selected from among four multiregression linear models employing forward selection and backward elimination algorithms. Next, the models will be used to predict the number of log IMDB votes and the log box office for a randomly selected film.
