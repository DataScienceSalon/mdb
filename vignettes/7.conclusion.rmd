...
The (log) number of IMDB votes grows linearly with cast scores; however,  box office grows at an increasing rate as cast popularity increases. A one fold increase in cast scores from 500 to 1000 produces a `r round(min(subset(scores, scores > 1000, select = dbo)) / min(subset(scores, scores > 500, select = dbo)) - 1, 0)` fold increase in box office revenue potential. 
However, doubling cast scores to 2000 points produces a `r round(min(subset(scores, scores > 2000, select = dbo)) / min(subset(scores, scores > 1000, select = dbo)) - 1, 0)` fold increase in revenue potential. The return on the investment in cast is substantial.
...
```{r votes}
obs <- c(1, 3, 10, 11, 18, 19)
knitr::kable(rLogBoxOffice$tests[obs, ], digits = 2) %>%
  kableExtra::kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = T, position = "center")
```
`r kfigr::figr(label = "votes", prefix = TRUE, link = TRUE, type="Table")` reveals a correlation of r = `r rLogBoxOffice$tests$Correlation[1]`, n = `r nrow(mData$numeric)`, p = `r rLogBoxOffice$tests[1,5]` for the log number of IMDB votes. Note that movie runtime emerged as a better predictor of (log) box office revenue than critics score, imdb rating and audience scores. In fact they were **negatively** correlated with (log)  box office revenue. This was not expected. 

## Genre
The multiregression analysis showed that genre accounts for `r round(modelA$anova %>% filter(Term == 'genre') %>% select(7), 2)`% of total variance and  `r round(modelA$anova %>% filter(Term == 'genre') %>% select(7) / sum(modelA$anova[1:(nrow(modelA$anova),7]-1) * 100, 2)`% of the variance attributed to the terms. `r kfigr::figr(label = "genres", prefix = TRUE, link = TRUE, type="Figure")` reveals (log) IMDB votes and (log)  box office by genre, with all other values (except the intercept) set to zero.
...
In order of influence, science fiction and fantasy films are the most popular, followed by "other", then action and adventure. The effect on the (log) number of votes was linear by genre; however, the revenue effect was nearly exponential. All else equal, science fiction & fantasy and action and adventure films have a revenue potential `r genre$pct[1]`%, and `r genre$pct[3]`% greater than the mean, respectively. 
