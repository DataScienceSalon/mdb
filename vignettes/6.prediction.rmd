...
cases <- test[complete.cases(test), ]
set.seed(233)
case <- cases[sample(1:nrow(cases), 1), ]
revenue <- prediction(mod =modelOne$best$mod, mName = "Model One", case = case, units = "dollars per day")
votes <- prediction(mod =modelA$mod, mName = "Model Alpha", case = case, units = "IMDB votes")
case$imdb_num_votes_log <- votes$yHat
revenue2 <- prediction(mod =modelOne$best$mod, mName = "Model One", case = case, units = "dollars per day")
