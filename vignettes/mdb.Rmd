...
cData <- correlate(x = mData, y = mData$dependent, yLab = "Log IMDB Votes", plot = FALSE)
knitr::kable(head(cData$tests, 10), digits = 2) %>%  
  kableExtra::kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = T, position = "center")
```
...
A plot of the correlations among the remaining variables was rendered in `r kfigr::figr(label = "cplot", prefix = TRUE, link = TRUE, type="Figure")`. 

```{r cplot}
cData <- correlate(x = fm$quantitative, y = fm$dependent, yLab = "Log IMDB Votes", plot = TRUE)
```
...
Thus, a few more highly correlated variables were revealed and the following were to avoid multicollinearity in the linear model.  
* scores - too highly correlated with IMDB rating
* audience score - too highly correlated with IMDB rating
...
### Forward Selection
The forward selection process begins with a null model then all variables are added to the model, one-by-one, and the model that provides the greatest improvement in adjusted R-squared is selected. If the adjusted R-squared for this set is greater than the best R-squared for the model, it is selected.  The process repeats until all predictors have been evaluated.
